import { Pilar } from "../interface";

export const pilar40Array: Pilar[] = [
  {
    id: 1,
    bacaArab: "‘Adaalah",
    skor: 0,
    namaArab: "العَدَالَة",
    namaIndo: "adil",
    tulisan: "‘Adaalah / العَدَالَة (adil)",
    labelDiri: "bersikap adil",
    definisi:
      "bersikap tengah-tengah, tepat dalam menempatkan sesuatu pada tempatnya tanpa berlebihan dan tidak kurang",
    profesi:
      "Hakim, Petugas pembinaan keluarga, Konselor masalah keluarga, Mediator, Petugas kontrol terhadap kesesuaian atas standar seperti kepatuhan kepada aturan, Quantity Surveyor, Petugas Commisioning atau peran yang bisa memiliki kekuatan untuk menyamakan aturan main dan peran lainnya  yang terkait dengan sifat adil",
    jurusanStudi:
      "Hukum Islam, Syariah, Konseling keluarga, Hukum Publik, Hukum Perdata, Hukum Internasioanal, Ilmu Hukum, Hukum Pidana, atau jurusan lainnya yang terkait dengan keadilan",
    akibatLalai: {
      namaArab: "الظُّلْم",
      tulisan: "Zhulm (الظُّلْم)(zhalim)",
      definisi:
        "berbuat tidak adil atau berat sebelah dalam bersikap dan bertindak sehingga merugikan orang atau pihak lain",
      perbaikan:
        "Sifat tercela zhulm (الظُّلْم)(zhalim) yang ditimbulkan akibat meremehkan bakat ‘adaalah, dapat diperbaiki dengan menguatkan bakat ‘adaalah itu sendiri dan bakat lainnya, yaitu antara lain bakat itsaar, rahmah, shabr, hilm, anaah, dan amaanah",
    },
    perbaikanLalai: [
      "العَدَالَة",
      " الاِيْثَار",
      " الرَّحْمَة",
      " الصَّبْر",
      " الحِلْم",
      " الاَنَاة",
      " الاَمَانَة",
    ],
    akibatBerlebihan: {
      namaArab: "الجَهْل",
      tulisan: "Jahl (الجَهْل) (kebodohan) ",
      definisi:
        "ketiadaan pengetahuan sehingga menganggap yang dilakukan adalah adil, tetapi sebenarnya yang dilakukannya bukanlah keadilan",
      perbaikan:
        "sifat tercela jahl (الجَهْل) (kebodohan) yang ditimbulkan akibat berlebihan dalam bakat ‘adaalah, dapat diperbaiki dengan menguatkan bakat syajaa’ah, munaafasah, dan ghairah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " المُنَافَسَة", " الغَيْرَة"],
    pernyataan:
      "Ketika menghadapi orang lain, aku tidak berat sebelah dalam bersikap, tidak memihak, bersikap tengah-tengah, adil, dan fair dalam “permainan”.",
  },
  {
    id: 2,
    bacaArab: "Amaanah",
    skor: 0,
    namaArab: "الاَمَانَة",
    namaIndo: "bertanggung jawab",
    tulisan: "Amaanah / الاَمَانَة (tanggung Jawab)",
    labelDiri: "bertanggung jawab terhadap tugas yang diberikan kepadanya",
    definisi:
      "senang jika diberi tanggung jawab serta menunaikan akad atau janji yang disepakati dengan sebaik-baiknya",
    profesi:
      "Pengelola Lembaga Pendidikan, Pengelola Baitul Maal, Pengasuh Anak Yatim, Pedagang, Pengelola Dana Umat, Bendahara, Account Sales, Manajer Umum, Manajer Keuangan, Quality Controller, Keamanan, dan peran lainnya  yang terkait dengan sifat amanah atau tanggung jawab",
    jurusanStudi:
      "Keguruan/tarbiyah, akuntansi, bisnis dan manajemen, administrasi",
    akibatLalai: {
      namaArab: "الخِيَانَة",
      tulisan: "Khiaanah (الخِيَانَة) (penghianatan)",
      definisi:
        "perkataan dan perlakuan yang tidak sesuai dengan apa yang telah diamanahkan",
      perbaikan:
        "Sifat tercela khiaanah (الخِيَانَة) (penghianatan) yang ditimbulkan akibat meremehkan bakat amaanah, dapat diperbaiki dengan menguatkan bakat itu sendiri dan bakat lainnya, yaitu antara lain bakat  iffah dan shidq.",
    },
    perbaikanLalai: ["الاَمَانَة", " الصِّدْق", " العِفَّة"],
    akibatBerlebihan: {
      namaArab: "التَّقْلِيْد",
      tulisan: "Taqliid (التَّقْلِيْد) (fanatik buta)",
      definisi:
        "menerima tugas dan menuruti perkataan dan perbuatan orang lain tanpa mempedulikan kebenarannya",
      perbaikan:
        "sifat tercela taqliid (التَّقْلِيْد) (fanatik buta) yang ditimbulkan akibat berlebihan dalam bakat amaanah, dapat diperbaiki dengan menguatkan bakat izzah dan waqaar.",
    },
    perbaikanBerlebihan: ["العِزَّة", " الوَقَار"],
    pernyataan:
      "Aku senang jika diberi tugas atau tanggung jawab atau amanah, dan aku berusaha melaksanakannya dengan sebaik-baiknya.",
  },
  {
    id: 3,
    bacaArab: "Anaah",
    skor: 0,
    namaArab: "الاَنَاة",
    namaIndo: "tidak tergesa",
    tulisan: "Anaah / الاَنَاة (tidak tergesa)",
    labelDiri: "mempertimbangkan dulu sebelum bertindak",
    definisi:
      "tenang, dan mencari kejelasan terlebih dahulu sebelum melakukan tindakan",
    profesi:
      "Mufti, Penasehat, Konsultan, Perumus peraturan, Perancang Sistem Keamanan, Pengawas, Pilot, Urusan Legal, Membuat Kontrak Bisnis yang baik atau memastikan kesesuaian dengan peraturan atau standar atau kode atau juga peran yang terkait dengan masalah keuangan dan atau keamanan",
    jurusanStudi:
      "Teknik Penerbangan, Teknik Mesin, Teknik Konstruksi Bangunan, Keselamatan Lalu Lintas, Teknik Listrik, Akuntansi, Manajemen Keuangan, dan jurusan lain yang terkait dengan kehati-hatian, keamanan, dan ketelitian",
    akibatLalai: {
      namaArab: "العَجَلَة",
      tulisan: "‘ajalah (العَجَلَة) (tergesa-gesa)",
      definisi:
        "keinginan untuk mendapatkan sesuatu sebelum tiba waktunya yang disebabkan oleh besarnya keinginannya terhadap sesuatu tersebut, seperti halnya orang yang memanen buah sebelum datang waktu panennya",
      perbaikan:
        "Sifat tercela ‘ajalah (العَجَلَة) (tergesa-gesa) yang ditimbulkan akibat meremehkan bakat anaah, dapat diperbaiki dengan menguatkan bakat itu sendiri dan bakat lainnya, yaitu antara lain bakat hilm.",
    },
    perbaikanLalai: ["الاَنَاة", " الحِلْم"],
    akibatBerlebihan: {
      namaArab: "وَسْوَاس",
      tulisan: "Was-was (وَسْوَاس)(ragu-ragu)",
      definisi:
        "salah satu senjata iblis yang disematkan di hati hamba Allah untuk menimbulkan keraguan, baik dalam beribadah, muamalah, maupun lainnya",
      perbaikan:
        "sifat tercela was-was (وَسْوَاس)(ragu-ragu) yang ditimbulkan akibat berlebihan dalam bakat anaah, dapat diperbaiki dengan menguatkan bakat husnuzhan  dan firaasah.",
    },
    perbaikanBerlebihan: ["حُسْنُ الظَّن", " الفِرَاسَة"],
    pernyataan:
      "Aku tidak tergesa-gesa ketika akan bertindak dan terlebih dahulu mempertimbangkan berbagai hal agar tidak terjadi kesalahan.",
  },
  {
    id: 4,
    bacaArab: "‘Aziimah",
    skor: 0,
    namaArab: "العَزِيمَة",
    namaIndo: "bertekad kuat",
    tulisan: "‘Aziimah / العَزِيمَة (tekad)",
    labelDiri: "memiliki tekad untuk segera memulai sesuatu pekerjaan",
    definisi:
      "memiliki tekad kuat untuk segera memulai suatu tindakan atau amalan",
    profesi:
      "Perintis atau pelopor, Team penggerak, Entrepreneur, Sales, Pengelola usaha-usaha baru,  atau usaha yang memerlukan perubahan besar",
    jurusanStudi:
      "Manajemen dan Bisnis, Teknologi Terapan, Marketing, Kewirausahaan, Pekerjaan Umum",
    akibatLalai: {
      namaArab: "الكَسَل",
      tulisan: "Kasal (الكَسَل) (malas)",
      definisi:
        "merasa berat dan lamban terhadap sesuatu, padahal memiliki kekuatan, karena tidak adanya keinginan untuk berbuat kebaikan",
      perbaikan:
        "Sifat tercela kasal (الكَسَل) (malas) yang ditimbulkan akibat meremehkan bakat ‘aziimah, dapat diperbaiki dengan menguatkan bakat itu sendiri dan bakat lainnya, yaitu antara lain bakat himmah dan nasyaath.",
    },
    perbaikanLalai: ["العَزِيمَة", " النَّشَاط", " الهِمَّة"],
    akibatBerlebihan: {
      namaArab: "الطَّمَع",
      tulisan: "Thama’ (الطَّمَع) (Serakah)",
      definisi:
        "keinginan kuat terhadap sesuatu yang sebenarnya tidak dibutuhkannya",
      perbaikan:
        "sifat tercela thama’ (الطَّمَع) (Serakah) yang ditimbulkan akibat berlebihan dalam bakat ‘aziimah, dapat diperbaiki dengan menguatkan bakat qanaa’ah, tawaadhu’, dan hayaa’.",
    },
    perbaikanBerlebihan: ["القَنَاعَة", " التَّوَاضُع", " الحَيَاء"],
    pernyataan:
      "Ketika akan melakukan suatu pekerjaan, aku memiliki tekad yang kuat untuk segera memulainya dan kadang seperti orang yang tidak sabaran.",
  },
  {
    id: 5,
    bacaArab: "Basyaasyah",
    skor: 0,
    namaArab: "البَشَاشَة",
    namaIndo: "berseri-seri",
    tulisan: "Basyaasyah / البَشَاشَة (berseri-seri)",
    labelDiri: "berseri-seri tampak wajahnya",
    definisi:
      "berseri-seri wajahnya, murah senyum, berkata lembut, dan memberikan penyambutan yang baik karena rasa gembira ketika bertemu orang lain",
    profesi:
      "Guru, Konselor, Presenter, Trainer, Motivator, Pemandu Kegiatan Anak, Pelayan Publik, Tour Leader (Pemandu Wisata)",
    jurusanStudi:
      "Ilmu Keguruan, Ilmu Komunikasi, PGTK (Pendidikan Guru TK), Keperawatan, Psikologi, Kepariwisataan, Pelayanan Publik",
    akibatLalai: {
      namaArab: "العَبُوْس",
      tulisan: "Abush (العَبُوْس) (muka masam)",
      definisi:
        "berwajah masam dalam bergaul sehingga membuat orang-orang tidak suka, takut, bahkan lari darinya",
      perbaikan:
        "Sifat tercela abush (العَبُوْس) (muka masam) yang ditimbulkan akibat meremehkan bakat basyaasyah, dapat diperbaiki dengan menguatkan bakat itu sendiri dan bakat lainnya, yaitu antara lain bakat rifq, muzaah, dan ulfah.",
    },
    perbaikanLalai: ["البَشَاشَة", " الرِّفْق", " المُزَاح", " الاُلْفَة"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ)(Lemah)",
      definisi:
        "kelemahan untuk mempertahankan sesuatu sehingga cenderung untuk direndahkan orang lain karena kurangnya ketegasan dalam bersikap",
      perbaikan:
        "sifat tercela dzull (الذُّلّ)(Lemah) yang ditimbulkan akibat berlebihan dalam bakat basyaasyah, dapat diperbaiki dengan menguatkan bakat syajaa’ah, munaafasah, dan ghairah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " المُنَافَسَة", " الغَيْرَة"],
    pernyataan:
      "Aku senang beramah tamah dan tersenyum kepada orang lain, berkata lembut, dan senang memberikan penyambutan sebaik-baiknya.",
  },
  {
    id: 6,
    bacaArab: "Dzakaa’",
    skor: 0,
    namaArab: "الذَّكَاء",
    namaIndo: "cerdas",
    tulisan: "Dzakaa’ / الذَّكَاء (cerdas)",
    labelDiri: "cerdas dalam menganalisa sesuatu",
    definisi:
      "memiliki kecepatan dan ketajaman berpikir dalam memahami, menghafal, menganalisa, dan menyimpulkan sesuatu yang dihadapkan kepadanya",
    profesi:
      "Ahli ilmu (Agama atau umum: Fisika, kimia, dll), Analis, Periset (teknologi, pemasaran, keuangan, atau kesehatan), Manajemen Database, Editor, Manajemen Risiko, Accounting, Programmer",
    jurusanStudi:
      "Jurusan Ilmu Hadits, Ilmu Al Qur’an, Fiqih, atau ilmu Agama lainnya, Teknik kimia, Fisika, Matematika, Teknik Komputer,  IT Programmer, Akuntansi",
    akibatLalai: {
      namaArab: "الجَهْل",
      tulisan: "Jahl (الجَهْل) (bodoh)",
      definisi:
        "meyakini sesuatu yang bertentangan dengan hakikat yang sebenarnya",
      perbaikan:
        "Sifat tercela jahl (الجَهْل) (bodoh) yang ditimbulkan akibat meremehkan bakat dzakaa’, dapat diperbaiki dengan menguatkan bakat itu sendiri dan bakat lainnya, yaitu antara lain bakat hikmah,",
    },
    perbaikanLalai: ["الذَّكَاء", " الحِكْمَة"],
    akibatBerlebihan: {
      namaArab: "اَهْلُ الرَّأْي",
      tulisan: "Ahlur ra’yi (اَهْلُ الرَّأْي) (pemuja akal)",
      definisi:
        "mendewakan logika dan akal dalam menghukumi sesuatu sehingga tidak menerima kebenaran yang tidak dapat dilogika",
      perbaikan:
        "sifat tercela ahlur ra’yi (اَهْلُ الرَّأْي) (pemuja akal) yang ditimbulkan akibat berlebihan dalam bakat dzakaa’, dapat diperbaiki dengan menguatkan bakat tawaadhu’, qanaa’ah, dan hayaa’.",
    },
    perbaikanBerlebihan: ["التَّوَاضُع", " القَنَاعَة", " الحَيَاء"],
    pernyataan:
      "Aku termasuk orang yang cepat dan mudah dalam memahami sesuatu yang dihadapkan kepadaku, seperti menghitung, menganalisa dll.",
  },
  {
    id: 7,
    bacaArab: "Fashaahah",
    skor: 0,
    namaArab: "الفَصَاحَة",
    namaIndo: "komunikatif",
    tulisan: "Fashaahah / الفَصَاحَة (fasih bicara)",
    labelDiri: "fasih berbicara dalam menjelaskan sesuatu",
    definisi:
      "memiliki kemampuan untuk menyampaikan sesuatu dengan ungkapan sederhana, meskipun sebenarnya rumit, sehingga mudah untuk dipahami",
    profesi:
      "Guru Tajwid Qira’ah, Qari’, Da’i, Guru (pengajar), Dosen, Motivator, Penyuluh Masyarakat, Public Relation, Duta, Sales, Marketing, Humas, Juru Bicara, Presenter, MC, Pengacara, Layanan Pelanggan, Penulis",
    jurusanStudi:
      "Pendidikan Agama, Konseling Keluarga, Keguruan, Ilmu Komunikasi, Hubungan Internasional, Marketing, Manajemen, Manajemen Bisnis",
    akibatLalai: {
      namaArab: "العُجْمَة",
      tulisan: "‘Ujmah (العُجْمَة) (gagap)",
      definisi:
        "kesalahan berbicara yang berupa pengulangan bunyi, suku kata, atau kata-kata, sehingga menjadikan tidak fasih berbicara",
      perbaikan:
        "Sifat tercela ‘ujmah (العُجْمَة) (gagap) yang ditimbulkan akibat meremehkan bakat fashaahah, dapat diperbaiki dengan menguatkan bakat fashaahah itu sendiri dan bakat lainnya, yaitu antara lain bakat syajaa’ah.",
    },
    perbaikanLalai: ["الفَصَاحَة", " الشَّجَاعَة"],
    akibatBerlebihan: {
      namaArab: "الجِدَال",
      tulisan: "Jidaal (الجِدَال) (berdebat)",
      definisi:
        "berbantah-bantahan untuk saling menjatuhkan satu dengan lainnya dengan menyalahgunakan kefasihan berbahasa yang dimiliki",
      perbaikan:
        "sifat tercela jidaal (الجِدَال) (berdebat) yang ditimbulkan akibat berlebihan dalam bakat fashaahah, dapat diperbaiki dengan menguatkan bakat itsaar, hilm, rahmah, dan shabr.",
    },
    perbaikanBerlebihan: ["الحِلْم", " الاِيْثَار", " الرَّحْمَة", " الصَّبْر"],
    pernyataan:
      "Aku mampu menjelaskan sesuatu yang rumit menjadi mudah dipahami meskipun dengan ungkapan yang sederhana, baik lisan maupun tulisan.",
  },
  {
    id: 8,
    bacaArab: "Firaasah",
    skor: 0,
    namaArab: "الفِرَاسَة",
    namaIndo: "berfirasat",
    tulisan: "Firaasah / الفِرَاسَة (firasat)",
    labelDiri: "memiliki firasat terhadap sesuatu yang akan terjadi",
    definisi:
      "memiliki ketajaman ilmu yang mampu menyimpulkan perkara batiniyah dengan memperhatikan tanda-tanda lahiriyah yang dapat dilihat, didengar, atau dirasakan",
    profesi:
      "Penasehat, Konsultan, Perumus Visi Misi, Entrepreneur, Event Organizer (EO), Tour Leader, Manajer",
    jurusanStudi: "Ilmu Aqidah, Ilmu Ushuluddin, Ilmu Jiwa, Manajemen",
    akibatLalai: {
      namaArab: "السَّفَه",
      tulisan: "Safah (السَّفَه) (bodoh)",
      definisi:
        "keyakinan terhadap sesuatu yang bertentangan dengan hakikatnya, sehingga dapat menjadikan seseorang tidak mampu memahami kebenaran yang sangat jelas tanda-tandanya",
      perbaikan:
        "Sifat tercela ‘ujmah safah (السَّفَه) (bodoh) yang ditimbulkan akibat meremehkan bakat firaasah, dapat diperbaiki dengan menguatkan bakat itu sendiri dan bakat lainnya, yaitu antara lain bakat hikmah dan dzakaa’.",
    },
    perbaikanLalai: ["الفِرَاسَة", " الحِكْمَة", " الذَّكَاء"],
    akibatBerlebihan: {
      namaArab: "الكَذِب",
      tulisan: "Kadzib (الكَذِب) (dusta)",
      definisi:
        "mengabarkan sesuatu tidak sesuai dengan yang sebenarnya, baik sengaja atau tidak, baik masa lalu atau masa mendatang",
      perbaikan:
        "sifat tercela kadzib (الكَذِب) (dusta) yang ditimbulkan akibat berlebihan dalam bakat firaasah, dapat diperbaiki dengan menguatkan bakat shidq, shamt, dan iffah.",
    },
    perbaikanBerlebihan: ["الصِّدْق", " الصَّمْت", " العِفَّة"],
    pernyataan:
      "Aku memiliki firasat kuat sehingga mampu menduga sesuatu yang akan terjadi dengan memperhatikan tanda-tanda yang tampak.",
  },
  {
    id: 9,
    bacaArab: "Ghairah",
    skor: 0,
    namaArab: "الغَيْرَة",
    namaIndo: "pencemburu",
    tulisan: "Ghairah / الغَيْرَة (cemburu)",
    labelDiri: "memiliki rasa cemburu",
    definisi:
      "memiliki rasa tidak senang jika syariat, aturan, hak, atau ketentuan lain yang telah ditetapkan dilanggar oleh orang lain",
    profesi:
      "Penegak Peraturan, Penegak Disiplin, Pengawas, Mandor, Evaluator, Auditor, Supervisor, Manajer.",
    jurusanStudi:
      "Administrasi, manajemen, manajemen bisnis, Sekolah Tinggi Pemerintahan Dalam Negeri, Akademi Militer, Akademi Kepolisian",
    akibatLalai: {
      namaArab: "الدِّيَاثَة",
      tulisan: "Diyaatsah (الدِّيَاثَة) (permisif)",
      definisi:
        "tidak ada kepedulian pada diri seorang laki-laki (dan juga perempuan) terhadap kemungkaran yang terjadi dalam keluarganya, lembaga, lingkungan masyarakat, atau lainnya",
      perbaikan:
        "Sifat tercela diyaatsah (الدِّيَاثَة) (permisif) yang ditimbulkan akibat meremehkan bakat ghairah, dapat diperbaiki dengan menguatkan bakat ghairah itu sendiri dan bakat lainnya, yaitu antara lain bakat munaafasah dan syajaa’ah.",
    },
    perbaikanLalai: ["الغَيْرَة", " المُنَافَسَة", " الشَّجَاعَة"],
    akibatBerlebihan: {
      namaArab: "الحَسَد",
      tulisan: "Hasad (الحَسَد) (iri hati)",
      definisi:
        "mengharap hilangnya nikmat pada orang yang diiri dan berpindah kepada orang yang iri",
      perbaikan:
        "sifat tercela hasad (الحَسَد) (iri hati) yang ditimbulkan akibat berlebihan dalam bakat ghairah, dapat diperbaiki dengan menguatkan bakat tawaadhu’, qanaa’ah, dan hayaa’.",
    },
    perbaikanBerlebihan: ["التَّوَاضُع", " القَنَاعَة", " الحَيَاء"],
    pernyataan:
      "Aku tidak senang jika ada yang menyaingiku, melanggar hakku, atau ada yang melanggar aturan dan kesepakatan yang telah ditetapkan.",
  },
  {
    id: 10,
    bacaArab: "Hayaa’",
    skor: 0,
    namaArab: "الحَيَاء",
    namaIndo: "pemalu",
    tulisan: "Hayaa’ / الحَيَاء (malu)",
    labelDiri: "memiliki perasaan malu",
    definisi:
      "merasa betapa besar pemberian/hak yang diterimanya, namun merasa dirinya masih memiliki banyak keterbatasan dan kekurangan, sehingga merasa tidak pantas untuk mendapatkannya",
    profesi:
      "Ahli Hikmah, Sastrawan, Penulis, Designer Fashion, Designer Interior, Arsitek, Kaligrafer, dsb.",
    jurusanStudi:
      "Ilmu Jiwa, Sastra, Arsitektur, Tata Busana, Seni, Desain Komunikasi Visual, Animasi, Fotografi, dsb.",
    akibatLalai: {
      namaArab: "الوَقَاحَة",
      tulisan: "Waqaahah/الوَقَاحَة (tidak tahu malu)",
      definisi:
        "kadang disebut juga ber “muka tebal”, yaitu tidak malu ketika melanggar syariat Allah",
      perbaikan:
        "Sifat tercela waqaahah/الوَقَاحَة (tidak tahu malu) yang ditimbulkan akibat meremehkan bakat hayaa’, dapat diperbaiki dengan menguatkan bakat hayaa’ itu sendiri dan bakat lainnya, yaitu antara lain bakat ‘iffah dan shidq.",
    },
    perbaikanLalai: ["الحَيَاء", " العِفَّة", " الصِّدْق"],
    akibatBerlebihan: {
      namaArab: "الدُوْنِيَّة",
      tulisan: "Duuniyyah/الدُوْنِيَّة (rendah diri) atau minder",
      definisi:
        "tekanan jiwa yang muncul karena adanya konflik dalam diri antara keinginan untuk menjadi unggul dan rasa takut karena merasa dirinya tidak memiliki kemampuan. Dengan kata lain dirinya merasa lebih rendah dari orang lain tetapi takut dianggap rendah",
      perbaikan:
        "sifat tercela duuniyyah/الدُوْنِيَّة (rendah diri) atau minder yang ditimbulkan akibat berlebihan dalam bakat hayaa’, dapat diperbaiki dengan menguatkan bakat himmah, aziimah, nasyaath, ihsaan, izzah, dan waqaar.",
    },
    perbaikanBerlebihan: [
      "الهِمَّة",
      " العَزِيمَة",
      " النَّشَاط",
      " الاِحْسَان",
      " العِزَّة",
      " الوَقَار",
    ],
    pernyataan:
      "Aku merasa memiliki kekurangan dan merasa apa yang telah aku terima melebihi hakku sehingga menjadikan aku merasa malu.",
  },
  {
    id: 11,
    bacaArab: "Hikmah",
    skor: 0,
    namaArab: "الحِكْمَة",
    namaIndo: "ahli hikmah",
    tulisan: "Hikmah / الحِكْمَة  (hikmah)",
    labelDiri: "berpikir sangat mendalam untuk mengambil hikmah darinya",
    definisi:
      "memiliki keilmuan yang mendalam tentang suatu pengetahuan hingga dapat menempatkan sesuatu pada kondisi yang sebenarnya",
    profesi:
      "Kosultan, Konselor, Mufti, Da’i/yah, Psikolog, Petugas Pembinaan Keluarga, Leader dalam membangun team yang berbeda kelompok, atau membantu orang agar merasa berguna",
    jurusanStudi:
      "Ilmu Aqidah, Ilmu Jiwa, Ilmu Adab, Sastra, Syariah, Psikologi, Manajemen",
    akibatLalai: {
      namaArab: "الجَل",
      tulisan: "Jahl/الجَل (kebodohan)",
      definisi:
        "meyakini sesuatu yang tidak sesuai dengan keadaan sebenarnya, atau tidak mengetahui sesuatu yang sebenarnya",
      perbaikan:
        "Sifat tercela jahl/الجَل )kebodohan( yang ditimbulkan akibat meremehkan bakat hikmah, dapat diperbaiki dengan menguatkan bakat hikmah itu sendiri dan bakat lainnya, yaitu antara lain bakat dzakaa’, nubl, husnuzhan, dan firaasah.",
    },
    perbaikanLalai: [
      "الحِكْمَة",
      " الذَّكَاء",
      " النُّبْل",
      " حُسْنُ الظَّن",
      " الفِرَاسَة",
    ],
    akibatBerlebihan: {
      namaArab: "الكَذِب",
      tulisan: "Kadzib (الكَذِب) (dusta)",
      definisi:
        "mengabarkan tentang sesuatu peristiwa yang tidak sesuai dengan keadaan sebenarnya, hanya didasari prasangka saja",
      perbaikan:
        "sifat tercela kadzib (الكَذِب) (dusta) yang ditimbulkan akibat berlebihan dalam bakat hikmah, dapat diperbaiki dengan menguatkan bakat shidq dan ‘iffah.",
    },
    perbaikanBerlebihan: ["الصِّدْق", " العِفَّة"],
    pernyataan:
      "Aku senang memikirkan sesuatu secara mendalam sehingga aku mampu menemukan hikmah dari sesuatu (peristiwa) yang telah terjadi.",
  },
  {
    id: 12,
    bacaArab: "Hilm",
    skor: 0,
    namaArab: "الحِلْم",
    namaIndo: "santun",
    tulisan: "Hilm / الحِلْم (santun)",
    labelDiri: "mampu untuk tidak membalas perbuatan orang lain",
    definisi:
      "memiliki kemampuan menahan diri untuk tidak memarahi, membalas, atau memberikan hukuman walaupun mampu melakukannya",
    profesi:
      "Pelayanan Masyarakat Umum, Rahabilitasi Kenakalan Remaja, Perawat Rumah Sakit, Guru (pengajar), Pelayanan Pelanggan (Customer Service), Sales, Manajer, Supervisor",
    jurusanStudi:
      "Keperawatan, Ilmu Komunikasi, Keguruan, Kepariwisataan, Marketing, Manajemen",
    akibatLalai: {
      namaArab: "الغَضَب",
      tulisan: "Ghadhab (الغَضَب) (marah)",
      definisi:
        "mendidihnya darah dalam hati untuk menahan orang yang mengganggunya atau untuk membalas dendam setelah mengganggunya",
      perbaikan:
        "Sifat tercela ghadhab (الغَضَب) (marah) yang ditimbulkan akibat meremehkan bakat hilm, dapat diperbaiki dengan menguatkan bakat hilm itu sendiri dan bakat lainnya, yaitu antara lain bakat shabr dan anaah.",
    },
    perbaikanLalai: ["الحِلْم", " الصَّبْر", " الاَنَاة"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "kelemahan untuk mempertahankan sesuatu yang seharusnya dipertahankan",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat hilm, dapat diperbaiki dengan menguatkan bakat syajaa’ah, munaafasah, dan ghairah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " المُنَافَسَة", " الغَيْرَة"],
    pernyataan:
      "Aku mampu menahan diri untuk tidak memarahi atau membalas orang yang bersalah meskipun aku mampu melakukannya, dan aku tetap santun.",
  },
  {
    id: 13,
    bacaArab: "Himmah",
    skor: 0,
    namaArab: "الهِمَّة",
    namaIndo: "bercita-cita tinggi",
    tulisan: "Himmah / الهِمَّة (cita-cita tinggi)",
    labelDiri: "memiliki cita-cita yang tinggi",
    definisi:
      "memiliki rasa ingin tahu dan gairah yang sangat tinggi untuk mengetahui dan meraih cita-cita yang tertinggi",
    profesi:
      "Team Perumus Visi Misi, Team Litbang Pendidikan atau lembaga lainnya, Pemimpin, Entrepreneur, Perencana Jangka Panjang, Visioner, Pengembang produk baru, Pengembang Perusahaan",
    jurusanStudi: "Ekonomi Syariah, Manajemen, Marketing, Manajemen Lingkungan",
    akibatLalai: {
      namaArab: "الفُتُوْر",
      tulisan: "Futuur (الفُتُوْر) (lemah kemauan)",
      definisi:
        "sikap malas, lesu, lamban dalam meraih sesuatu, padahal sebelumnya bersungguh-sungguh, semangat, dan bergairah",
      perbaikan:
        "Sifat tercela futuur (الفُتُوْر) (lemah kemauan) yang ditimbulkan akibat meremehkan bakat himmah, dapat diperbaiki dengan menguatkan bakat himmah itu sendiri dan bakat lainnya, yaitu antara lain bakat ‘aziimah dan nasyaath.",
    },
    perbaikanLalai: ["الهِمَّة", " العَزِيمَة", " النَّشَاط"],
    akibatBerlebihan: {
      namaArab: "طُوْلُ الأَمَلِ",
      tulisan: "Thuulul amal (طُوْلُ الأَمَلِ) (panjang angan-angan)",
      definisi:
        "keinginan kuat terhadap sesuatu yang kemungkinan kecil untuk dapat diraih",
      perbaikan:
        "sifat tercela thuulul amal (طُوْلُ الأَمَلِ) (panjang angan-angan) yang ditimbulkan akibat berlebihan dalam bakat himmah, dapat diperbaiki dengan menguatkan bakat  qanaa’ah, tawaadhu’, dan hayaa’.",
    },
    perbaikanBerlebihan: ["القَنَاعَة", " التَّوَاضُع", " الحَيَاء"],
    pernyataan:
      "Aku memiliki rasa ingin tahu yang tinggi terhadap sesuatu sehingga aku memiliki gairah untuk meraih mimpi (cita-cita) yang tertinggi.",
  },
  {
    id: 14,
    bacaArab: "Husnuzhan",
    skor: 0,
    namaArab: "حُسْنُ الظَّن",
    namaIndo: "berprasangka baik",
    tulisan: "husnuzhan / حُسْنُ الظَّن (prasangka baik)",
    labelDiri: "berpikir positif tentang segala sesuatu",
    definisi:
      "memiliki kecenderungan menduga yang baik dari pada yang buruk walaupun fakta yang dilihatnya mendukung kepada dugaan yang buruk",
    profesi:
      "Hakim, Pendidik, Guru, Pengajar, Juru Damai, Mediator, Relator, Pelayanan Publik",
    jurusanStudi:
      "Hukum Islam, Keguruan, Manajemen, Ilmu Komunikasi, Manajemen Bisnis",
    akibatLalai: {
      namaArab: "سُوْءُ الظَّن",
      tulisan: "Suu’uzhan (سُوْءُ الظَّن) (prasangka buruk)",
      definisi:
        "memenuhi hati dengan prasangka buruk, sampai memenuhi juga lisannya dan anggota badannya",
      perbaikan:
        "Sifat tercela su’uzhan (سُوْءُ الظَّن) (prasangka buruk) yang ditimbulkan akibat meremehkan bakat husnuzhan, dapat diperbaiki dengan menguatkan bakat husnuzhan itu sendiri dan bakat lainnya, yaitu antara lain bakat hikmah, dan firaasah.",
    },
    perbaikanLalai: ["حُسْنُ الظَّن", " الحِكْمَة", " الفِرَاسَة"],
    akibatBerlebihan: {
      namaArab: "التَّقْلِيْد",
      tulisan: "Taqliid (التَّقْلِيْد) (fanatik buta)",
      definisi:
        "mengikuti dan mempercayai perkataan dan perbuatan orang lain tanpa mempedulikan kebenarannya",
      perbaikan:
        "sifat tercela taqliid (التَّقْلِيْد) (fanatik buta) yang ditimbulkan akibat berlebihan dalam bakat husnuzhan, dapat diperbaiki dengan menguatkan bakat  himmah, aziimah, ‘izzah, dan waqaar.",
    },
    perbaikanBerlebihan: ["العِزَّة", " العَزِيمَة", " الهِمَّة", " الوَقَار"],
    pernyataan:
      "Aku selalu berprasangka baik terhadap orang lain meskipun fakta yang aku ketahui mendukung kepada dugaan yang buruk.",
  },
  {
    id: 15,
    bacaArab: "‘Iffah",
    skor: 0,
    namaArab: "العِفَّة",
    namaIndo: "menjaga diri",
    tulisan: "‘Iffah / العِفَّة (jaga diri)",
    labelDiri: "mampu menahan diri untuk tidak menuruti hasrat",
    definisi:
      "mampu menahan diri terhadap dorongan hawa nafsu dalam dirinya sehingga tidak melakukan hal yang dilarang, baik dalam harta, ucapan, atau perbuatan",
    profesi:
      "Pengelola Keuangan Umat, Bendahara, Team Sarana Prasarana, Team Belanja, Pemimpin, Akuntan",
    jurusanStudi:
      "Syariah, Hukum Islam, Manajemen Keuangan, Akuntansi, Administrasi",
    akibatLalai: {
      namaArab: "الفَحْش",
      tulisan: "Fahsy (الفَحْش) (keji)",
      definisi:
        "segala hal yang mengindikasikan pada wilayah keburukan, kemaksiatan, dan dosa yang keluar pada wilayah batas kewajaran, serta dipandang sangat hina oleh akal sehat manusia dan syariat Islam",
      perbaikan:
        "Sifat tercela fahsy (الفَحْش) (keji) yang ditimbulkan akibat meremehkan bakat ‘iffah, dapat diperbaiki dengan menguatkan bakat ‘iffah itu sendiri dan bakat lainnya, yaitu antara lain bakat hayaa’, tawaadhu’, qanaa’ah dan shidq.",
    },
    perbaikanLalai: [
      "العِفَّة",
      " الحَيَاء",
      " التَّوَاضُع",
      " القَنَاعَة",
      " الصِّدْق",
    ],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "ketundukan jiwa dikarenakan ketidakmampuan untuk mempertahankan sesuatu, sehingga merendahkan diri sendiri",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat ‘iffah, dapat diperbaiki dengan menguatkan bakat  himmah, aziimah, ‘izzah, dan waqaar.",
    },
    perbaikanBerlebihan: ["الهِمَّة", " العَزِيمَة", " العِزَّة", " الوَقَار"],
    pernyataan:
      "Aku mampu menjaga diri sehingga aku sangat menyesal jika terlanjur melakukan suatu pelanggaran atau larangan karena menuruti hasratku.",
  },
  {
    id: 16,
    bacaArab: "Ihsaan",
    skor: 0,
    namaArab: "الاِحْسَان",
    namaIndo: "perfeksionis",
    tulisan: "Ihsaan / الاِحْسَان (perfeksionis)",
    labelDiri: "perfeksionis",
    definisi:
      "mampu berbuat yang terbaik dalam amalan ibadah maupun dalam memberikan manfaat terhadap diri, orang lain, atau lainnya",
    profesi:
      "Team Penjamin Mutu Pendidikan, Quality Controller (QC), Peran untuk membantu orang hebat menjadi sukses seperti Pelatih, Manajer, Mentor, Guru, Transformational leader",
    jurusanStudi:
      "Tadriibud Du’aat, Hadits, Fiqih, Syariah, Manajemen, Manajemen Bisnis, Ilmu Sains dan Teknologi",
    akibatLalai: {
      namaArab: "الإِسَاءة",
      tulisan: "Isaa’ah (الإِسَاءة) (berbuat kerusakan)",
      definisi:
        "perbuatan yang menjadikan sesuatu menjadi lebih buruk atau perbuatan yang merusak",
      perbaikan:
        "Sifat tercela isaa’ah (الإِسَاءة) (berbuat kerusakan) yang ditimbulkan akibat meremehkan bakat ihsaan, dapat diperbaiki dengan menguatkan bakat ihsaan itu sendiri dan bakat lainnya, yaitu antara lain bakat himmah, ‘aziimah, nasyaath, ‘izzah, dan waqaar.",
    },
    perbaikanLalai: [
      "الاِحْسَان",
      " الهِمَّة",
      " العَزِيمَة",
      " النَّشَاط",
      " العِزَّة",
      " الوَقَار",
    ],
    akibatBerlebihan: {
      namaArab: "التَّبْذِيْر",
      tulisan: "Tabdziir (التَّبْذِيْر) (boros, berlebihan)",
      definisi:
        "membelanjakan atau melakukan sesuatu pada hal yang seharusnya tidak diperlukan, berlebihan dalam membelanjakan atau berperilaku",
      perbaikan:
        "sifat tercela tabdziir (التَّبْذِيْر) (boros, berlebihan) yang ditimbulkan akibat berlebihan dalam bakat ihsaan, dapat diperbaiki dengan menguatkan bakat  qanaa’ah dan tawaadhu’.",
    },
    perbaikanBerlebihan: ["القَنَاعَة", " التَّوَاضُع"],
    pernyataan:
      "Aku memiliki sifat perfeksionis sehingga aku selalu ingin segala sesuatu harus sempurna dan lebih sempurna lagi, dan aku tidak ingin seadanya.",
  },
  {
    id: 17,
    bacaArab: "Itsaar",
    skor: 0,
    namaArab: "الاِيْثَار",
    namaIndo: "melayani",
    tulisan: "Itsaar /الاِيْثَار (melayani)",
    labelDiri: "suka mendahulukan orang lain",
    definisi:
      "memiliki kecenderungan untuk lebih mendahulukan orang lain dari pada dirinya sendiri dan rela berkorban dalam memberikan manfaat atau dalam menghindari sesuatu, walaupun dirinya lebih membutuhkannya",
    profesi:
      "Da’i/da’iyah, Pengelola lembaga sosial seperti panti asuhan, Laziz, Baitul Maal; Konselor, Pelayanan Pelanggan, Maintenance, Perawat, Pekerja Sosial, Relawan, Team Palang Merah",
    jurusanStudi:
      "Tadriibud Du’aat, Tarbiyah, Penyuluh Masyarakat, Kesehatan Masyarakat, Keperawatan, Kebidanan, Kedokteran, Kepalangmerahan",
    akibatLalai: {
      namaArab: "البُخْل",
      tulisan: "Bakhil (البُخْل) (bakhil)",
      definisi:
        "menahan kepemilikan yang seharusnya tidak dicegah, menahan sesuatu yang seharusnya tidak ditahan, atau menahan sesuatu yang wajib",
      perbaikan:
        "Sifat tercela bukhl (البُخْل) (bakhil) yang ditimbulkan akibat meremehkan bakat itsaar, dapat diperbaiki dengan menguatkan bakat itsaar itu sendiri dan bakat lainnya, yaitu antara lain bakat juud, nushrah, dan nashiihah.",
    },
    perbaikanLalai: ["الاِيْثَار", " الجُوْد", " النُّصْرَة", " النَّصِيْحَة"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "kelemahan jiwa dikarenakan ketidakmampuan untuk mempertahankan hak pribadinya",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat itsaar, dapat diperbaiki dengan menguatkan bakat syajaa’ah, munaafasah, dan ghairah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " المُنَافَسَة", " الغَيْرَة"],
    pernyataan:
      "Aku senang mendahulukan orang lain meskipun sebenarnya aku lebih membutuhkannya sehingga aku menjadi orang yang senang melayani.",
  },
  {
    id: 18,
    bacaArab: "‘Izzah",
    skor: 0,
    namaArab: "العِزَّة",
    namaIndo: "harga diri",
    tulisan: "‘Izzah / العِزَّة (harga diri)",
    labelDiri: "mampu mempertahankan harga diri",
    definisi:
      "mampu bersikap kuat, tegas, dan tangguh dalam mempertahankan kemuliaan atau harga dirinya, sehingga tidak direndahkan dan dapat mengalahkan pihak lain",
    profesi:
      "Utusan, Duta Besar, Da’i, Da’iyah, Guru, Pengajar, Pembawa Misi, Leader",
    jurusanStudi:
      "Ilmu Aqidah, Syariah, Fiqih, Tadriibud Du’aat, Keguruan, Hubungan Internasional, Manajemen",
    akibatLalai: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi: "lemahnya jiwa sehingga tidak mampu mempertahankan prinsipnya",
      perbaikan:
        "Sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat meremehkan bakat ‘izzah, dapat diperbaiki dengan menguatkan bakat ‘izzah itu sendiri dan bakat lainnya, yaitu antara lain bakat himmah, ‘aziimah, nasyaath, ihsaan, dan waqaar",
    },
    perbaikanLalai: [
      "العِزَّة",
      " الهِمَّة",
      " العَزِيمَة",
      " النَّشَاط",
      " الاِحْسَان",
      " الوَقَار",
    ],
    akibatBerlebihan: {
      namaArab: "الكِبْر",
      tulisan: "Kibr (الكِبْر) (sombong)",
      definisi:
        "karena gengsi dan egonya, sehingga menolak kebenaran dan merendahkan orang lain",
      perbaikan:
        "sifat tercela kibr (الكِبْر) (sombong) yang ditimbulkan akibat berlebihan dalam bakat ‘izzah, dapat diperbaiki dengan menguatkan bakat   tawaadhu’, qanaa’ah, dan hayaa’.",
    },
    perbaikanBerlebihan: ["التَّوَاضُع", " القَنَاعَة", " الحَيَاء"],
    pernyataan:
      "Aku memiliki prinsip yang kujadikan sebagai harga diriku dan aku mampu mempertahankannya meskipun dalam tekanan yang sangat berat.",
  },
  {
    id: 19,
    bacaArab: "Juud",
    skor: 0,
    namaArab: "الجُوْد",
    namaIndo: "dermawan",
    tulisan: "Juud / الجُوْد (dermawan)",
    labelDiri: "dermawan terhadap orang lain",
    definisi:
      "senang memberikan sesuatu tanpa diminta dan tidak mengharap ucapan terima kasih atau bentuk balasan lainnya",
    profesi:
      "Relawan, Petugas Sosial, Konselor, Perawat, Pengelola LAZIZ, Pengelola Baitul Maal, Panitia Bulan Amal, Entrepreneur, Leader, manajer",
    jurusanStudi:
      "Pendidikan Agama Islam, Ilmu Fiqih, Manajemen, Keperawatan, Sosiologi, Bimbingan Konseling, Manajemen Bisnis",
    akibatLalai: {
      namaArab: "البُخْل",
      tulisan: "Bukhl (البُخْل) (kikir)",
      definisi:
        "menahan sesuatu yang seharusnya tidak ditahan bahkan yang seharusnya wajib dikeluarkan",
      perbaikan:
        "Sifat tercela bukhl (البُخْل)(kikir) yang ditimbulkan akibat meremehkan bakat juud, dapat diperbaiki dengan menguatkan bakat juud itu sendiri dan bakat lainnya, yaitu antara lain bakat nushrah dan nashiihah.",
    },
    perbaikanLalai: ["الجُوْد", " النُّصْرَة", " النَّصِيْحَة"],
    akibatBerlebihan: {
      namaArab: "التَّبْذِيْر",
      tulisan: "Tabdzir (التَّبْذِيْر) (boros)",
      definisi:
        "mengeluarkan sesuatu pada hal-hal yang seharusnya tidak terlalu perlu dan berlebihan dalam membelanjakan sesuatu",
      perbaikan:
        "sifat tercela tabdzir (التَّبْذِيْر) (boros) yang ditimbulkan akibat berlebihan dalam bakat juud, dapat diperbaiki dengan menguatkan bakat   tawaadhu’, qanaa’ah, dan hayaa’.",
    },
    perbaikanBerlebihan: ["القَنَاعَة", " التَّوَاضُع", " الحَيَاء"],
    pernyataan:
      "Aku senang memberikan sesuatu tanpa diminta kepada siapapun dan aku tidak mengharap balasan atau ucapan terima kasih.",
  },
  {
    id: 20,
    bacaArab: "Kitmaanus sirr",
    skor: 0,
    namaArab: "كِتْمَانُ السِّرِّ",
    namaIndo: "penjaga rahasia",
    tulisan: "Kitmaanus Sirr / كِتْمَانُ السِّرِّ (jaga rahasia)",
    labelDiri: "mampu menjaga rahasia meskipun bukan aib",
    definisi:
      "mampu menutupi suatu kabar (bukan aib) agar tidak tersebar untuk mencegah timbulnya keburukan jika tersebar",
    profesi:
      "Hakim, Pengacara, Konselor, Intelijen, Administrator, Pengelola Dokumen Negara, Psikolog, Kepolisian, Militer, Keuangan Negara",
    jurusanStudi:
      "Hukum Islam, Syariah, Sekolah Tinggi Pertahanan Nasional, Politeknik Keuangan Negara, Akademi Kepolisian, Akademi Militer, Administrasi, Akuntansi",
    akibatLalai: {
      namaArab: "إِفْشَاءُ السِّرّ",
      tulisan: "Ifsyaa’us sirr  (إِفْشَاءُ السِّرّ) (membocorkan rahasia)",
      definisi:
        "membocorkan rahasia orang yang memberikan kepercayaan kepadanya",
      perbaikan:
        "Sifat tercela ifsyaa’us sirr  (إِفْشَاءُ السِّرّ) (membocorkan rahasia) yang ditimbulkan akibat meremehkan bakat kitmaanus sirr, dapat diperbaiki dengan menguatkan bakat kitmaanus sirr itu sendiri dan bakat lainnya, yaitu antara lain bakat amaanah dan satr.",
    },
    perbaikanLalai: ["كِتْمَانُ السِّرِّ", " الاَمَانَة", " السَّتْر"],
    akibatBerlebihan: {
      namaArab: "البُخْل",
      tulisan: "Bukhl (البُخْل) (bakhil)",
      definisi:
        "menahan sesuatu yang seharusnya tidak ditahan untuk dikabarkan",
      perbaikan:
        "sifat tercela bukhl (البُخْل) (bakhil) yang ditimbulkan akibat berlebihan dalam bakat kitmaanus sirr, dapat diperbaiki dengan menguatkan bakat juud, nushrah, nashiihah dan fashaahah.",
    },
    perbaikanBerlebihan: [
      "الجُوْد",
      " النُّصْرَة",
      " النَّصِيْحَة",
      " الفَصَاحَة",
    ],
    pernyataan:
      "Aku mampu menahan diri untuk tidak menyebarkan kabar (yang bukan aib) karena aku anggap sebagai privasi bagi orang yang bersangkutan.",
  },
  {
    id: 21,
    bacaArab: "Mahabbah",
    skor: 0,
    namaArab: "المَحَبَّة",
    namaIndo: "penuh cinta",
    tulisan: "Mahabbah / المَحَبَّة (penuh cinta)",
    labelDiri: "suka memperdalam hubungan yang telah terjalin",
    definisi:
      "memiliki kecintaan yang mendalam dan khusus terhadap sesuatu untuk memperkuat dan meperdalam hubungan",
    profesi:
      "Guru Privat, Terapist, Konselor, Psikolog, Guru PAUD, Peternak Hewan Rumahan, Perawat",
    jurusanStudi:
      "Tarbiyah, Pendidikan Guru TK (PGTK), Keguruan, Pendidikan Terapist, Keperawatan, Peternakan",
    akibatLalai: {
      namaArab: "الكَرَاهِيَة",
      tulisan: "Karaahiyah (الكَرَاهِيَة) (kebencian)",
      definisi:
        "ketidaksukaan menjalin kedekatan dalam suatu hubungan sehingga mengakibatkan kerenggangan hubungan",
      perbaikan:
        "Sifat tercela karaahiyah (الكَرَاهِيَة) (kebencian) yang ditimbulkan akibat meremehkan bakat mahabbah, dapat diperbaiki dengan menguatkan bakat mahabbah itu sendiri dan bakat lainnya, yaitu antara lain bakat ulfah, ta’aawun dan basyaasyah.",
    },
    perbaikanLalai: ["المَحَبَّة", " الاُلْفَة", " التَّعَاوُن", " البَشَاشَة"],
    akibatBerlebihan: {
      namaArab: "التَّقْلِيْد",
      tulisan: "Taqliid (التَّقْلِيْد) (fanatik buta)",
      definisi:
        "mencintai, mengikuti dan mempercayai orang lain atau sesuatu tanpa mempedulikan kebenaran dan manfaatnya",
      perbaikan:
        "sifat tercela taqliid (التَّقْلِيْد) (fanatik buta) yang ditimbulkan akibat berlebihan dalam bakat mahabbah, dapat diperbaiki dengan menguatkan bakat izzah dan waqaar.",
    },
    perbaikanBerlebihan: ["العِزَّة", " الوَقَار"],
    pernyataan:
      "Aku senang memperdalam hubungan yang sudah terjalin, dengan memberikan kasih sayang secara khusus kepada yang bersangkutan.",
  },
  {
    id: 22,
    bacaArab: "Munaafasah",
    skor: 0,
    namaArab: "المُنَافَسَة",
    namaIndo: "kompetitif",
    tulisan: "Munaafasah / المُنَافَسَة (kompetisi)",
    labelDiri: "suka bersaing dengan orang lain",
    definisi:
      "senang bersaing dengan usaha semaksimal mungkin agar dirinya menjadi yang paling unggul dari yang lainnya",
    profesi:
      "Bidang Periklanan, Motivator, Competition Organizer, Team Litbang Perusahaan, Konsultan atau pengelola perusahaan dalam menghadapi persaingan, Sales, Pelatih Olahraga",
    jurusanStudi:
      "Manajemen, Manajemen Bisnis, Olahraga dan Kesehatan, Sekolah Pelatih",
    akibatLalai: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "tidak mampu untuk bangkit  atau ketakutan dalam menghadapi persaingan dalam kehidupan sehingga menjadi terpuruk",
      perbaikan:
        "Sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat meremehkan bakat munaafasah, dapat diperbaiki dengan menguatkan bakat munaafasah itu sendiri dan bakat lainnya, yaitu antara lain bakat ghairah, syajaa’ah, himmah, ihsaan, dan nasyaath.",
    },
    perbaikanLalai: [
      "المُنَافَسَة",
      " الغَيْرَة",
      " الشَّجَاعَة",
      " الهِمَّة",
      " الاِحْسَان",
      " النَّشَاط",
    ],
    akibatBerlebihan: {
      namaArab: "الظُّلْم",
      tulisan: "Zhulm (الظُّلْم)(zhalim)",
      definisi:
        "berbuat aniaya dan curang kepada orang atau pihak lain hanya karena ingin unggul dari yang lain",
      perbaikan:
        "sifat tercela zhulm (الظُّلْم)(zhalim) yang ditimbulkan akibat berlebihan dalam bakat munaafasah, dapat diperbaiki dengan menguatkan bakat ‘adaalah, itsaar, rahmah, qanaa’ah, dan tawaadhu’.",
    },
    perbaikanBerlebihan: [
      "العَدَالَة",
      " الاِيْثَار",
      " الرَّحْمَة",
      " القَنَاعَة",
      " التَّوَاضُع",
    ],
    pernyataan:
      "Aku senang bersaing, berlomba atau berkompetisi agar aku menjadi yang paling unggul dari yang lainnya.",
  },
  {
    id: 23,
    bacaArab: "Muzaah",
    skor: 0,
    namaArab: "المُزَاح",
    namaIndo: "humoris",
    tulisan: "Muzaah / المُزَاح (canda)",
    labelDiri: "suka bercanda untuk mengakrabkan hubungan",
    definisi:
      "senang bercanda tapi tidak menyinggung orang lain, santai dalam bergaul",
    profesi: "MC, Pranata cara (Jawa), Penerima Tamu, Motivator, Comentator",
    jurusanStudi: "Ilmu Komunikasi, Marketing, Keguruan",
    akibatLalai: {
      namaArab: "الجَفَاء",
      tulisan: "Jafaa’ (الجَفَاء) (kaku)",
      definisi:
        "bersikap kaku dalam pergaulan sehingga dapat mengurangi keakraban",
      perbaikan:
        "Sifat tercela jafaa’ (الجَفَاء) (kaku) yang ditimbulkan akibat meremehkan bakat muzaah, dapat diperbaiki dengan menguatkan bakat muzaah itu sendiri dan bakat lainnya, yaitu antara lain bakat basyaasyah, rifq, dan mahabbah.",
    },
    perbaikanLalai: ["المُزَاح", " البَشَاشَة", " الرِّفْق", " المَحَبَّة"],
    akibatBerlebihan: {
      namaArab: "الاِسْتِهْزَاء",
      tulisan: "Istihzaa’ (الاِسْتِهْزَاء) (mengejek)",
      definisi:
        "berkata dan berbuat untuk tujuan bermain-main, bukan untuk keseriusan, dengan mengejek sifat dan perilaku orang yang diejek",
      perbaikan:
        "sifat tercela istihzaa’ (الاِسْتِهْزَاء) (mengejek) yang ditimbulkan akibat berlebihan dalam bakat muzaah, dapat diperbaiki dengan menguatkan bakat shamt, ‘iffah, dan shidq.",
    },
    perbaikanBerlebihan: ["الصَّمْت", " العِفَّة", " الصِّدْق"],
    pernyataan:
      "Aku senang bercanda dan mengungkapkan kata-kata jenaka sehingga suasana menjadi lebih ceria dan lebih akrab.",
  },
  {
    id: 24,
    bacaArab: "Nashiihah",
    skor: 0,
    namaArab: "النَّصِيْحَة",
    namaIndo: "penasehat",
    tulisan: "Nashiihah / النَّصِيْحَة (nasehat)",
    labelDiri: "suka menasehati orang lain",
    definisi:
      "senang memberikan nasehat, saran, dorongan kepada orang lain agar menjadi lebih baik",
    profesi:
      "Penasehat, Da’i, Da’iyah, Guru, Konselor, Konsultan, Pemimpin, Manajer, Leader, Supervisor, Evaluator, Pengawas, Pelatih, Pelayanan Pelanggan (Customer Service)",
    jurusanStudi:
      "Ilmu Agama, Ilmu Komunikasi, Ilmu Manajemen, Ilmu Hukum, Psikologi, Bimbingan Konseling",
    akibatLalai: {
      namaArab: "الغِشِّ",
      tulisan: "Ghisy (الغِشِّ)(menipu)",
      definisi:
        "menjerumuskan dan membiarkan seseorang menjadi lebih buruk keadaannya. (Sebagaimana yang dilakukan iblis kepada Adam dan Hawa)",
      perbaikan:
        "Sifat tercela ghisy (الغِشِّ)(menipu) yang ditimbulkan akibat meremehkan bakat nashiihah, dapat diperbaiki dengan menguatkan bakat nashiihah itu sendiri dan bakat lainnya, yaitu antara lain bakat nushrah dan juud.",
    },
    perbaikanLalai: ["النَّصِيْحَة", " النُّصْرَة", " الجُوْد"],
    akibatBerlebihan: {
      namaArab: "التَّنْفِير",
      tulisan: "Tanfiir (التَّنْفِير) (menakut-nakuti)",
      definisi:
        "menakut-nakuti orang lain sehingga menjadi bosan, lari atau enggan terhadap sesuatu yang dinasehatkan",
      perbaikan:
        "sifat tercela tanfiir (التَّنْفِير) (menakut-nakuti) yang ditimbulkan akibat berlebihan dalam bakat nashiihah, dapat diperbaiki dengan menguatkan bakat rifq, mahabbah, rahmah, dan itsaar.",
    },
    perbaikanBerlebihan: [
      "الرِّفْق",
      " المَحَبَّة",
      " الرَّحْمَة",
      " الاِيْثَار",
    ],
    pernyataan:
      "Aku senang memberikan motivasi atau dorongan, saran-saran atau nasehat kepada orang lain agar menjadi lebih baik.",
  },
  {
    id: 25,
    bacaArab: "Nasyaath",
    skor: 0,
    namaArab: "النَّشَاط",
    namaIndo: "bersemangat",
    tulisan: "Nasyaath / النَّشَاط (semangat)",
    labelDiri: "bersemangat menyelesaikan pekerjaan",
    definisi:
      "memiliki semangat dan bekerja keras untuk menyelesaikan apa yang sedang dilakukan",
    profesi:
      "Penyelenggara Lembaga pendidikan, Pemborong Proyek, Tenaga Penjual/Sales, Teknisi Proyek, Teknisi Lapangan, Pekerja Lapangan, Relawan, Petugas SAR, Pemadam Kebakaran",
    jurusanStudi:
      "Teknik Sipil, Teknik Permesinan, Teknik Instalasi dan Tenaga Listrik, Teknik perkayuan, Teknik Otomotif, Keperawatan, Kebidanan, Marketing, dan jurusan lain yang banyak menumbuhkan sifat kerja keras",
    akibatLalai: {
      namaArab: "الكَسَل",
      tulisan: "Kasal (الكَسَل) (malas)",
      definisi:
        "merasa berat dan lamban terhadap sesuatu, padahal memiliki kekuatan, karena tidak adanya keinginan untuk berbuat kebaikan",
      perbaikan:
        "Sifat tercela kasal (الكَسَل) (malas) yang ditimbulkan akibat meremehkan bakat nasyaath, dapat diperbaiki dengan menguatkan bakat nasyaath itu sendiri dan bakat lainnya, yaitu antara lain bakat himmah, ihsaan, dan ‘aziimah.",
    },
    perbaikanLalai: ["النَّشَاط", " الهِمَّة", " الاِحْسَان", " العَزِيمَة"],
    akibatBerlebihan: {
      namaArab: "الطَّمَع",
      tulisan: "Thama’ (الطَّمَع) (serakah)",
      definisi:
        "keinginan kuat untuk mendapatkan sesuatu yang sebenarnya tidak dibutuhkannya",
      perbaikan:
        "sifat tercela thama’ (الطَّمَع) (serakah) yang ditimbulkan akibat berlebihan dalam bakat nasyaath, dapat diperbaiki dengan menguatkan bakat qanaa’ah dan tawaadhu’.",
    },
    perbaikanBerlebihan: ["القَنَاعَة", " التَّوَاضُع"],
    pernyataan:
      "Aku selalu ingin menyelesaikan suatu pekerjaan yang sedang aku lakukan dengan penuh semangat dan kerja keras.",
  },
  {
    id: 26,
    bacaArab: "Nubl",
    skor: 0,
    namaArab: "النُّبْل",
    namaIndo: "cerdik",
    tulisan: "Nubl / النُّبْل (cerdik)",
    labelDiri: "banyak akal untuk menemukan solusi masalah",
    definisi:
      "cepat mengerti (tentang situasi dan sebagainya) dan pandai mencari pemecahannya,  panjang atau banyak akalnya",
    profesi:
      "Konsultan, Konselor, Team Penanganan Masalah, Perencana Strategi, Manajer, Leader, Team Pencari Solusi Bagi Perusahaan/Lembaga Yang Bermasalah",
    jurusanStudi:
      "Ilmu Fiqih, Ekonomi Syariah, Manajemen, Sosiologi, Hubungan Internasional, atau jurusan lain yang banyak menumbuhkan kemampuan untuk menemukan solusi",
    akibatLalai: {
      namaArab: "الجَهْل",
      tulisan: "Jahl (الجَهْل) (kebodohan)",
      definisi:
        "kebingungan dalam menghadapi masalah karena tidak mengetahui solusi atau jalan keluar yang harus ditempuhnya",
      perbaikan:
        "Sifat tercela jahl (الجَهْل) (kebodohan) yang ditimbulkan akibat meremehkan bakat nubl, dapat diperbaiki dengan menguatkan bakat nubl itu sendiri dan bakat lainnya, yaitu antara lain bakat firaasah dan husnuzhan.",
    },
    perbaikanLalai: ["النُّبْل", " الفِرَاسَة", " حُسْنُ الظَّن"],
    akibatBerlebihan: {
      namaArab: "الغِشِّ",
      tulisan: "Ghisy (الغِشِّ)(licik)",
      definisi:
        "menipu orang lain dengan kemampuannya bersiasat yang tidak diketahui oleh orang lain",
      perbaikan:
        "sifat tercela Ghisy (الغِشِّ)(licik) yang ditimbulkan akibat berlebihan dalam bakat nubl, dapat diperbaiki dengan menguatkan bakat shidq, shamt, ‘iffah, dan hayaa’.",
    },
    perbaikanBerlebihan: ["الصِّدْق", " الصَّمْت", " العِفَّة", " الحَيَاء"],
    pernyataan:
      "Ketika aku mengalami suatu kesulitan, aku dengan mudah menemukan  peluang-peluang yang dapat dijadikan sebagai solusinya.",
  },
  {
    id: 27,
    bacaArab: "Nushrah",
    skor: 0,
    namaArab: "النُّصْرَة",
    namaIndo: "menolong",
    tulisan: "Nushrah / النُّصْرَة (menolong)",
    labelDiri: "suka menolong orang dari kesulitan",
    definisi:
      "senang membantu orang lain yang mengalami kesusahan (atau terzhalimi) hingga terlepas dari kesusahan tersebut",
    profesi:
      "Polisi, Penegak Hukum, Relawan, Pengacara, Lembaga Bantuan Hukum (LBH), Hakim, atau profesi lain yang terkait dengan bantuan terhadap orang yang terzhalimi",
    jurusanStudi:
      "PAI, Syari’ah, Ilmu Hukum, Ilmu Sosial, Akademi Kepolisian, Akademi Militer",
    akibatLalai: {
      namaArab: "الخِذْلَان",
      tulisan: "Khidzlaan (الخِذْلَان) (menerlantarkan)",
      definisi:
        "membiarkan, tidak peduli, tidak memelihara sesuatu yang seharusnya dipelihara",
      perbaikan:
        "Sifat tercela khidzlaan (الخِذْلَان) (menerlantarkan) yang ditimbulkan akibat meremehkan bakat nushrah, dapat diperbaiki dengan menguatkan bakat nushrah itu sendiri dan bakat lainnya, yaitu antara lain bakat juud dan nashiihah.",
    },
    perbaikanLalai: ["النُّصْرَة", " الجُوْد", " النَّصِيْحَة"],
    akibatBerlebihan: {
      namaArab: "الظُّلْم",
      tulisan: "Zhulm (الظُّلْم) (zhalim)",
      definisi: "meletakkan sesuatu bukan pada tempatnya",
      perbaikan:
        "sifat tercela zhulm (الظُّلْم) (zhalim) yang ditimbulkan akibat berlebihan dalam bakat nushrah, dapat diperbaiki dengan menguatkan bakat ‘adaalah, itsaar, rahmah, qanaa’ah, dan tawaadhu’.",
    },
    perbaikanBerlebihan: [
      "العَدَالَة",
      " الاِيْثَار",
      " الرَّحْمَة",
      " القَنَاعَة",
      " التَّوَاضُع",
    ],
    pernyataan:
      "Aku senang membantu orang lain yang mengalami kesusahan sampai  terlepas dari kesusahan tersebut.",
  },
  {
    id: 28,
    bacaArab: "Qanaa'ah",
    skor: 0,
    namaArab: "القَنَاعَة",
    namaIndo: "sederhana",
    tulisan: "Qanaa’ah /  القَنَاعَة (sederhana)",
    labelDiri: "merasa cukup dengan apa yang telah aku terima",
    definisi:
      "rela menerima apapun yang diberikan kepadanya dan merasa cukup terhadap apa yang telah diterimanya",
    profesi:
      "Team Perencana Anggaran Belanja Lembaga, Team Penghemat Anggaran Perusahaan, Petugas Lapangan, Explorer, Relawan",
    jurusanStudi:
      "Ilmu Ekonomi, Manajemen Bisnis, Ilmu Sosial, Ilmu Manajemen Sumber Daya Alam, Akuntansi, atau jurusan lain yang terkait dengan perhitungan nilai ekonomis",
    akibatLalai: {
      namaArab: "الطَّمَع",
      tulisan: "Thama’ (الطَّمَع) (serakah, tamak)",
      definisi:
        "berkeinginan kuat untuk mendapatkan sesuatu yang sebenarnya tidak dibutuhkannya",
      perbaikan:
        "Sifat tercela thama’ (الطَّمَع) (serakah, tamak) yang ditimbulkan akibat meremehkan bakat qanaa’ah, dapat diperbaiki dengan menguatkan bakat qanaa’ah itu sendiri dan bakat lainnya, yaitu antara lain bakat tawaadhu’.",
    },
    perbaikanLalai: ["القَنَاعَة", " التَّوَاضُع"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "ketidakmampuan untuk menuntut haknya, sehingga meyusahkan dirinya sendiri",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat qanaa’ah, dapat diperbaiki dengan menguatkan bakat himmah, ihsaan, aziimah, nasyaath, ‘izzah, dan waqaar.",
    },
    perbaikanBerlebihan: [
      "الهِمَّة",
      " الاِحْسَان",
      " العَزِيمَة",
      " النَّشَاط",
      " العِزَّة",
      " الوَقَار",
    ],
    pernyataan:
      "Aku merasa cukup dengan apa yang telah aku miliki dan rela menerima apapun yang diberikan kepadaku.",
  },
  {
    id: 29,
    bacaArab: "Rahmah",
    skor: 0,
    namaArab: "الرَّحْمَة",
    namaIndo: "berbelas kasih",
    tulisan: "Rahmah / الرَّحْمَة (belas kasih)",
    labelDiri: "berbelas kasihan kepada orang lain",
    definisi:
      "berbelas kasihan dan bersimpati karena ingin memberikan yang terbaik bagi yang dibelas kasihi",
    profesi:
      "Konselor, Pembinaan Keluarga Sejahtera, Guru PAUD-TK, Psikolog, Sales, HRD, Perawat, Operator Telepon, Psikiater, Dispatcher, Layanan Pelanggan",
    jurusanStudi:
      "PAI, Keguruan, Tarbiyah, Ilmu Sosial, Manajemen, Manajemen Bisnis, Psikologi, atau jurusan lain yang terkait dengan penguatan empati atau perasaan",
    akibatLalai: {
      namaArab: "الغِلْظَة",
      tulisan: "Ghilzhah (الغِلْظَة)   (kasar)",
      definisi:
        "kejam, kurang belas kasih, tidak pengertian dan kurang hasrat untuk kebaikan",
      perbaikan:
        "Sifat tercela ghilzhah (الغِلْظَة) (kasar) yang ditimbulkan akibat meremehkan bakat rahmah, dapat diperbaiki dengan menguatkan bakat rahmah itu sendiri dan bakat lainnya, yaitu antara lain bakat itsaar dan hilm.",
    },
    perbaikanLalai: ["الرَّحْمَة", " الاِيْثَار", " الحِلْم"],
    akibatBerlebihan: {
      namaArab: "التَّقْلِيْد",
      tulisan: "Taqliid (التَّقْلِيْد) (fanatik buta)",
      definisi:
        "mengikuti dan menuruti perkataan dan perbuatan orang lain tanpa mempedulikan kebenarannya",
      perbaikan:
        "sifat tercela taqliid (التَّقْلِيْد) (fanatik buta) yang ditimbulkan akibat berlebihan dalam bakat rahmah, dapat diperbaiki dengan menguatkan bakat ‘izzah dan waqaar.",
    },
    perbaikanBerlebihan: ["العِزَّة", " الوَقَار"],
    pernyataan:
      "Aku mudah merasa kasihan dan bersimpati kepada orang yang mengalami kesusahan dan berkeinginan untuk memberikan yang terbaik kepadanya.",
  },
  {
    id: 30,
    bacaArab: "Rifq",
    skor: 0,
    namaArab: "الرِّفْق",
    namaIndo: "lemah lembut",
    tulisan: "Rifq / الرِّفْق (lemah lembut)",
    labelDiri: "lemah lembut dalam bergaul",
    definisi:
      "mampu mengambil sisi termudah dalam semua urusan, sehingga lemah-lembut dalam perkataan dan perbuatan",
    profesi:
      "Guru, Pendidik, Penasehat, Konsultan, Manajer, Supervisor, Pengawas, Marketing, Customer Service, Leader",
    jurusanStudi:
      "Syari’ah, Keguruan, Psikologi Islam, Manajemen, Manajemen Bisnis",
    akibatLalai: {
      namaArab: "العَنْفُ",
      tulisan: "‘Anfu (العَنْفُ) (kasar)",
      definisi: "bersikap kasar kepada orang lain yang melakukan kekeliruan",
      perbaikan:
        "Sifat tercela ‘anfu (العَنْفُ) (kasar) yang ditimbulkan akibat meremehkan bakat rifq, dapat diperbaiki dengan menguatkan bakat rifq itu sendiri dan bakat lainnya, yaitu antara lain bakat basyaasyah dan mahabbah.",
    },
    perbaikanLalai: ["الرِّفْق", " البَشَاشَة", " المَحَبَّة"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "kelemahan jiwa sehingga tidak mampu memperbaiki suatu kesalahan atau pelanggaran yang dilakukan oleh seseorang",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat rifq, dapat diperbaiki dengan menguatkan bakat syajaa’ah, ghairah, dan munaafasah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " الغَيْرَة", " المُنَافَسَة"],
    pernyataan:
      "Aku selalu mengambil sisi termudah dalam menyikapi semua urusan, sehingga aku selalu lemah lembut dalam berbuat maupun berkata.",
  },
  {
    id: 31,
    bacaArab: "Satr",
    skor: 0,
    namaArab: "السَّتْر",
    namaIndo: "menutup aib",
    tulisan: "Satr / السَّتْرُ (menutup aib)",
    labelDiri: "mampu menutupi aib diri sendiri atau orang lain",
    definisi:
      "mampu menutupi dan tidak menampakkan atau menceritakan aib diri sendiri atau orang lain",
    profesi: "Ajudan, Asisten Pribadi, Sekretaris, Agen Rahasia, Notaris",
    jurusanStudi:
      "Akademi Kepolisian, Akademi Militer, Ilmu Kenotariatan, Administrasi",
    akibatLalai: {
      namaArab: "الغِيْبَة",
      tulisan: "Ghiibah (الغِيْبَة) (menggunjing)",
      definisi:
        "menyebarkan aib diri (mujaahir) dan orang lain yang seharusnya ditutupi",
      perbaikan:
        "Sifat tercela ghiibah (الغِيْبَة) (menggunjing), yaitu menyebarkan aib diri (mujaahir) yang ditimbulkan akibat meremehkan bakat satr, dapat diperbaiki dengan menguatkan bakat satr itu sendiri dan bakat lainnya, yaitu antara lain bakat kitmaanus sirr, amaanah,  dan shabr.",
    },
    perbaikanLalai: [
      "السَّتْر",
      " كِتْمَانُ السِّرِّ",
      " الاَمَانَة",
      " الصَّبْر",
    ],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (Lemah)",
      definisi:
        "kelemahan untuk mengatakan sesuatu yang seharusnya dikatakan, sehingga menyembunyikan ilmu yang seharusnya disampaikan",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat satr, dapat diperbaiki dengan menguatkan bakat fashaahah dan nashiihah.",
    },
    perbaikanBerlebihan: ["الفَصَاحَة", " النَّصِيْحَة"],
    pernyataan:
      "Aku mampu menutupi dan tidak menampakkan atau menceritakan aib diri sendiri atau orang lain.",
  },
  {
    id: 32,
    bacaArab: "Shabr",
    skor: 0,
    namaArab: "الصَّبْر",
    namaIndo: "penyabar",
    tulisan: "Shabr / الصَّبْر (sabar)",
    labelDiri: "bersabar terhadap apa yang menimpa",
    definisi:
      "mampu menahan diri untuk tidak bereaksi (menolak/membalas) terhadap apa yang diterimanya, baik berupa tekanan dari luar, kesulitan hidup, maupun berupa hambatan dalam meraih apa yang diharapkan",
    profesi:
      "Da’i, Khadimul Ummah, Public Serving, Pengemban Misi, Pelayanan Pelanggan, Maintenance, Perawat, Pekerja Sosial, Relawan",
    jurusanStudi:
      "Syari’ah, Keperawatan, Kebidanan, Ilmu Sosial, Kepariwisataan, Manajemen",
    akibatLalai: {
      namaArab: "الجَزَع",
      tulisan: "Jaza’ (الجَزَع) (tidak sabar)",
      definisi:
        "tampaknya kegelisahan yang menjadikan seseorang bereaksi (menolak/membalas) perlakukan terhadap dirinya sehingga menimbulkan kerusakan yang lebih besar",
      perbaikan:
        "Sifat tercela jaza’ (الجَزَع) (tidak sabar) yang ditimbulkan akibat meremehkan bakat shabr, dapat diperbaiki dengan menguatkan bakat shabr itu sendiri dan bakat lainnya, yaitu antara lain bakat anaah  dan hilm.",
    },
    perbaikanLalai: ["الصَّبْر", " الاَنَاة", " الحِلْم"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "kelemahan untuk mempertahankan dan mendapatkan hak yang seharusnya diambilnya",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat shabr, dapat diperbaiki dengan menguatkan bakat syajaa’ah, ghairah, dan munaafasah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " الغَيْرَة", " المُنَافَسَة"],
    pernyataan:
      "Aku mampu menahan diri untuk tidak bereaksi dan sabar terhadap tekanan, kesulitan atau apapun yang menimpa diriku.",
  },
  {
    id: 33,
    bacaArab: "Shamt",
    skor: 0,
    namaArab: "الصَّمْت",
    namaIndo: "pendiam",
    tulisan: "Shamt / الصَّمْت (diam)",
    labelDiri: "mampu menjaga perkataan",
    definisi:
      "memiliki sikap banyak diam dan jika berbicara seperlunya atau yang bermanfaat saja",
    profesi:
      "Pelayan Umat, Ajudan, Pelaksana Lapangan, Penulis, Peneliti, Analis",
    jurusanStudi:
      "Teknik Kimia, Teknik Informatika (IT), Administrasi Perkantoran",
    akibatLalai: {
      namaArab: "الغِيْبَة",
      tulisan: "Ghiibah (الغِيْبَة) (menggunjing)",
      definisi:
        "membicarakan suatu hal pada diri seseorang, baik dengan perkataan, isyarat, atau menirukan, yang apabila dia mengetahuinya, maka dia tidak menyukainya",
      perbaikan:
        "Sifat tercela ghiibah (الغِيْبَة) (menggunjing) yang ditimbulkan akibat meremehkan bakat shamt, dapat diperbaiki dengan menguatkan bakat shamt itu sendiri dan bakat lainnya, yaitu antara lain bakat shidq, ‘iffah  dan hayaa’.",
    },
    perbaikanLalai: ["الصَّمْت", " الصِّدْق", " العِفَّة", " الحَيَاء"],
    akibatBerlebihan: {
      namaArab: "الجُبْن",
      tulisan: "Jubn (الجُبْن) (penakut)",
      definisi: "takut untuk berbicara yang seharusnya tidak perlu ditakuti",
      perbaikan:
        "sifat tercela jubn (الجُبْن) (penakut) yang ditimbulkan akibat berlebihan dalam bakat shamt, dapat diperbaiki dengan menguatkan bakat syajaa’ah, ghairah, dan munaafasah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " الغَيْرَة", " المُنَافَسَة"],
    pernyataan:
      "Aku cenderung banyak diam dan berbicara seperlunya, sehingga yang aku ucapkan hanya ucapan yang diperlukan saja.",
  },
  {
    id: 34,
    bacaArab: "Shidq",
    skor: 0,
    namaArab: "الصِّدْق",
    namaIndo: "jujur",
    tulisan: "Shidq / الصِّدْق (jujur)",
    labelDiri: "mampu berkata dan berbuat apa adanya",
    definisi:
      "memiliki sikap mudah menerima kebenaran serta berkata dan berbuat  sesuai dengan apa yang sebenarnya",
    profesi:
      "Pengelola Dana Umat (Donasi), Kurir/Duta, Bidang Keuangan, Pengelola Baitul Mal, Team Belanja (Pengadaan Sarana Prasarana), Hakim, Jaksa",
    jurusanStudi: "Administrasi Keuangan, Hukum Islam, Ilmu Hukum, Manajemen",
    akibatLalai: {
      namaArab: "الكَذِب",
      tulisan: "Kadzib (الكَذِب) (dusta)",
      definisi:
        "mengabarkan dan berperilaku sesuatu yang tidak sesuai dengan yang sebenarnya",
      perbaikan:
        "Sifat tercela kadzib (الكَذِب) (dusta) yang ditimbulkan akibat meremehkan bakat shidq, dapat diperbaiki dengan menguatkan bakat shidq itu sendiri dan bakat lainnya, yaitu antara lain bakat ‘iffah dan shamt.",
    },
    perbaikanLalai: ["الصِّدْق", " العِفَّة", " الصَّمْت"],
    akibatBerlebihan: {
      namaArab: "إِفْشَاءُ السِّرّ",
      tulisan: "Ifsyaa’us sirr (إِفْشَاءُ السِّرّ) (membocorkan rahasia)",
      definisi:
        "membocorkan rahasia orang yang memberikan kepercayaan kepadanya",
      perbaikan:
        "sifat tercela ifsyaa’us sirr (إِفْشَاءُ السِّرّ) (membocorkan rahasia) yang ditimbulkan akibat berlebihan dalam bakat shidq, dapat diperbaiki dengan menguatkan bakat ‘izzah, waqaar dan syajaa’ah.",
    },
    perbaikanBerlebihan: ["العِزَّة", " الوَقَار", " الشَّجَاعَة"],
    pernyataan:
      "Aku mudah menerima saran dan masukan serta berkata dan berbuat sesuai dengan apa yang sebenarnya.",
  },
  {
    id: 35,
    bacaArab: "Syajaa’ah",
    skor: 0,
    namaArab: "الشَّجَاعَة",
    namaIndo: "pemberani",
    tulisan: "Syajaa’ah / الشَّجَاعَة (berani)",
    labelDiri: "berani menghadapi orang secara langsung",
    definisi:
      "teguh dan berani dalam menghadapi orang untuk mengendalikannya, mengaturnya, dan menguasainya",
    profesi:
      "Tentara, TNI, Komandan, Pengusaha, Petugas Keamanan, Sales, Negosiator, Wartawan, Pengacara, HRD, Pembelian",
    jurusanStudi:
      "Ilmu Hukum Islam, Syariah, Akademi Militer, Akademi Kepolisian, Manajemen, Manajemen Bisnis",
    akibatLalai: {
      namaArab: "الجُبْن",
      tulisan: "Jubn (الجُبْن) (penakut)",
      definisi: "takut terhadap sesuatu yang seharusnya tidak perlu ditakuti",
      perbaikan:
        "Sifat tercela jubn (الجُبْن) (penakut) yang ditimbulkan akibat meremehkan bakat syajaa’ah, dapat diperbaiki dengan menguatkan bakat syajaa’ah itu sendiri dan bakat lainnya, yaitu antara lain bakat ghairah, munaafasah, himmah, ihsaan dan ‘aziimah.",
    },
    perbaikanLalai: [
      "الشَّجَاعَة",
      " الغَيْرَة",
      " المُنَافَسَة",
      " الهِمَّة",
      " الاِحْسَان",
      " العَزِيمَة",
    ],
    akibatBerlebihan: {
      namaArab: "التَّهَوُّر",
      tulisan: "Tahawwur (التَّهَوُّر) (ceroboh)",
      definisi:
        "bertindak tanpa dipikirkan dan dinalar terlebih dahulu, dan tidak peduli dengan apa yang akan terjadi",
      perbaikan:
        "sifat tercela tahawwur (التَّهَوُّر) (ceroboh) yang ditimbulkan akibat berlebihan dalam bakat syajaa’ah, dapat diperbaiki dengan menguatkan bakat shabr, hilm dan anaah.",
    },
    perbaikanBerlebihan: ["الصَّبْر", " الحِلْم", " الاَنَاة"],
    pernyataan:
      "Aku berani menghadapi orang secara langsung, untuk berbicara kepadanya, memerintah, mengatur, atau menguasainya.",
  },
  {
    id: 36,
    bacaArab: "Ta'aawun",
    skor: 0,
    namaArab: "التَّعَاوُن",
    namaIndo: "bekerjaasama",
    tulisan: "Ta’aawun / التَّعَاوُن (kerjasama)",
    labelDiri: "mampu bekerjasama dengan orang lain",
    definisi:
      "senang bekerjasama dengan orang lain untuk mencapai tujuan bersama",
    profesi:
      "Penggerak dakwah, Guru, Pembangun jaringan antara orang-orang dengan cara pandang yang berbeda, Juru Damai, Penasehat, Wirausahawan, Manajer",
    jurusanStudi: "Dakwah, Manajemen, Ilmu Komunikasi, Ilmu Ekonomi, Keguruan",
    akibatLalai: {
      namaArab: "العُدْوَان",
      tulisan: "‘Udwaan (العُدْوَان) (memusuhi)",
      definisi:
        "berupa ketidakadilan dalam bermuamalah, seperti merampas hak orang lain, menindas, dll",
      perbaikan:
        "Sifat tercela ‘udwaan (العُدْوَان) (memusuhi) yang ditimbulkan akibat meremehkan bakat ta’aawun, dapat diperbaiki dengan menguatkan bakat ta’aawun itu sendiri dan bakat lainnya, yaitu antara lain bakat ‘adaalah dan ulfah.",
    },
    perbaikanLalai: ["التَّعَاوُن", " العَدَالَة", " الاُلْفَة"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "ketundukan jiwa dikarenakan ketidakmampuan untuk mempertahankan sesuatu, sehingga menjadi tidak mandiri dan tergantung pada orang lain",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat ta’aawun, dapat diperbaiki dengan menguatkan bakat syajaa’ah, ghairah dan munaafasah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " الغَيْرَة", " المُنَافَسَة"],
    pernyataan:
      "Aku senang menjalin kerjasama dengan orang lain, berdamai, dan menghindari konflik, untuk mencapai tujuan bersama.",
  },
  {
    id: 37,
    bacaArab: "Tawaadhu'",
    skor: 0,
    namaArab: "التَّوَاضُع",
    namaIndo: "rendah hati",
    tulisan: "Tawaadhu’ / التَّوَاضُع (rendah hati)",
    labelDiri: "rendah hati terhadap kehebatan yang dimiliki",
    definisi:
      "memiliki rasa tidak suka menampakkan atau ditampakkan kelebihannya meskipun memiliki kemampuan dan kelebihan",
    profesi:
      "Guru TK, Konselor, Pemimpin, Relawan, Pengasuh Panti Asuhan, Penasehat, public serving",
    jurusanStudi:
      "Dakwah, Psikologi, Sastra, Sosiologi, Ilmu Komunikasi, Keguruan, Ilmu Kejiwaan",
    akibatLalai: {
      namaArab: "الكِبْر",
      tulisan: "Kibr (الكِبْر) (takabur atau sombong)",
      definisi:
        "melihat diri lebih besar dari orang lain, sehingga menolak kebenaran dan merendahkan orang lain, tidak menerima nasehat dan suka mencela kelemahan orang lain",
      perbaikan:
        "Sifat tercela kibr (الكِبْر) (takabur atau sombong) yang ditimbulkan akibat meremehkan bakat tawaadhu’, dapat diperbaiki dengan menguatkan bakat tawaadhu’ itu sendiri dan bakat lainnya, yaitu antara lain bakat qanaa’ah dan hayaa’.",
    },
    perbaikanLalai: ["التَّوَاضُع", " القَنَاعَة", " الحَيَاء"],
    akibatBerlebihan: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "ketidakmampuan untuk mempertahankan dan menuntut sesuatu yang menjadi haknya, sehingga menjadi dihinakan",
      perbaikan:
        "sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat berlebihan dalam bakat tawaadhu’, dapat diperbaiki dengan menguatkan bakat syajaa’ah, ghairah dan munaafasah.",
    },
    perbaikanBerlebihan: ["الشَّجَاعَة", " الغَيْرَة", " المُنَافَسَة"],
    pernyataan:
      "Aku tidak suka menampakkan atau ditampakkan kehebatanku meskipun aku memilikinya, sehingga ada orang lain yang menyebutku rendah hati.",
  },
  {
    id: 38,
    bacaArab: "Ulfah",
    skor: 0,
    namaArab: "الاُلْفَة",
    namaIndo: "bersatu",
    tulisan: "Ulfah / الاُلْفَة  (bersatu)",
    labelDiri: "suka berkumpul dan mengumpulkan orang",
    definisi: "senang bersatu dan berkumpul  karena adanya kesamaan",
    profesi:
      "Da’i, Marketing, Pengelola lembaga sosial, Motivator kelompok, Wakil suara minoritas, Pemimpin kelompok dengan latar budaya beragam, Mentor bagi mereka yang baru bergabung di dalam organisasi",
    jurusanStudi: "Dakwah, Manajemen Pendidikan, Manajemen Bisnis, Sosiologi",
    akibatLalai: {
      namaArab: "النَّمِيْمَة",
      tulisan: "Namiimah (النَّمِيْمَة) (adu domba)",
      definisi:
        "menyampaikan perkataan seseorang kepada orang lain dengan tujuan merusak hubungan di antara mereka",
      perbaikan:
        "Sifat tercela namiimah (النَّمِيْمَة) (adu domba) yang ditimbulkan akibat meremehkan bakat ulfah, dapat diperbaiki dengan menguatkan bakat ulfah itu sendiri dan bakat lainnya, yaitu antara lain bakat ta’aawun dan ‘adaalah.",
    },
    perbaikanLalai: ["الاُلْفَة", " التَّعَاوُن", " العَدَالَة"],
    akibatBerlebihan: {
      namaArab: "التَّقْلِيْد",
      tulisan: "Taqliid (التَّقْلِيْد) (fanatik buta)",
      definisi:
        "mengikuti dan mempercayai perkataan dan perbuatan kebanyakan orang di kelompoknya tanpa mempedulikan kebenarannya",
      perbaikan:
        "sifat tercela taqliid (التَّقْلِيْد) (fanatik buta) yang ditimbulkan akibat berlebihan dalam bakat ulfah, dapat diperbaiki dengan menguatkan bakat ‘izzah dan waqaar.",
    },
    perbaikanBerlebihan: ["العِزَّة", " الوَقَار"],
    pernyataan:
      "Aku senang berkumpul dengan orang banyak dan berusaha menambah anggota perkumpulan sebanyak-banyaknya.",
  },
  {
    id: 39,
    bacaArab: "Wafaa'",
    skor: 0,
    namaArab: "الوَفَاء",
    namaIndo: "menepati janji",
    tulisan: "Wafaa’ / الوَفَاء (tepat janji)",
    labelDiri: "teguh dalam memenuhi janji",
    definisi:
      "teguh memegang janji yang telah disepakati, walaupun kondisinya sebagai pihak yang dicurangi",
    profesi:
      "Pengajar Aqidah, Pelayanan Masyarakat, Pelayanan Pelanggan, Birokrat, Relator, Account Sales, Manajer, Keuangan, Quality Control, Keamanan",
    jurusanStudi:
      "Ilmu Aqidah, Akuntansi, Administrasi, Manajemen, Sosiologi, Ilmu Hukum",
    akibatLalai: {
      namaArab: "الغَدْر",
      tulisan: "Ghadr (الغَدْر) (langar janji)",
      definisi: "melanggar janji yang telah disepakati bersama",
      perbaikan:
        "Sifat tercela ghadr (الغَدْر) (langar janji) yang ditimbulkan akibat meremehkan bakat wafaa’, dapat diperbaiki dengan menguatkan bakat wafaa’ itu sendiri dan bakat lainnya, yaitu antara lain bakat shidq, hayaa’ dan ‘iffah.",
    },
    perbaikanLalai: ["الوَفَاء", " الصِّدْق", " الحَيَاء", " العِفَّة"],
    akibatBerlebihan: {
      namaArab: "التَّقْلِيْد",
      tulisan: "Taqliid (التَّقْلِيْد) (fanatik buta)",
      definisi:
        "mengikuti, mempercayai, dan menyepakati perjanjian dengan pihak lain tanpa mempedulikan kerugian dan kebenarannya",
      perbaikan:
        "sifat tercela taqliid (التَّقْلِيْد) (fanatik buta) yang ditimbulkan akibat berlebihan dalam bakat wafaa’, dapat diperbaiki dengan menguatkan bakat ‘izzah dan waqaar.",
    },
    perbaikanBerlebihan: ["العِزَّة", " الوَقَار"],
    pernyataan:
      "Aku memegang teguh janji yang telah disepakati bersama, meskipun posisiku sebagai pihak yang dicurangi.",
  },
  {
    id: 40,
    bacaArab: "Waqaar",
    skor: 0,
    namaArab: "الوَقَار",
    namaIndo: "berwibawa",
    tulisan: "Waqaar / الوَقَار (wibawa)",
    labelDiri: "berwibawa",
    definisi:
      "memiliki sikap tenang tapi kelihatan tangguh, tidak banyak bicara, tidak tergesa tapi mendahului dalam melakukan semua urusan",
    profesi:
      "Trainer, Pendamping pelatihan, Peningkatan SDM, Konsultan, Team penanganan masalah lembaga/perusahaan",
    jurusanStudi: "Teknik Sipil, Teknik Listrik, Teknik Mesin, Manajemen",
    akibatLalai: {
      namaArab: "الذُّلّ",
      tulisan: "Dzull (الذُّلّ) (lemah)",
      definisi:
        "tidak mampu untuk bangkit  atau ketakutan dalam menghadapi persaingan dalam kehidupan sehingga menjadi terpuruk",
      perbaikan:
        "Sifat tercela dzull (الذُّلّ) (lemah) yang ditimbulkan akibat meremehkan bakat waqaar, dapat diperbaiki dengan menguatkan bakat waqaar itu sendiri dan bakat lainnya, yaitu antara lain bakat Izzah dan Aziimah.",
    },
    perbaikanLalai: ["الوَقَار", " العِزَّة", " العَزِيمَة"],
    akibatBerlebihan: {
      namaArab: "العُجْب",
      tulisan: "Ujb (العُجْب) (bangga diri)",
      definisi:
        "persepsi tentang tingginya derajat pribadinya, yang sebenarnya tidak layak dinisbatkan kepadanya",
      perbaikan:
        "sifat tercela ujb (العُجْب) (bangga diri) yang ditimbulkan akibat berlebihan dalam bakat waqaar, dapat diperbaiki dengan menguatkan bakat tawaadhu’ dan qanaa’ah.",
    },
    perbaikanBerlebihan: ["التَّوَاضُع", " القَنَاعَة"],
    pernyataan:
      "Aku tampak tenang tapi tangguh, tidak banyak bicara tapi bisa memahamkan, tidak tergesa tapi mendahului dalam semua urusan.",
  },
];
